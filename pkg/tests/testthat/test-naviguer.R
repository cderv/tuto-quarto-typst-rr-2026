test_that("lister_exercices() renvoie les trois exercices", {
  codes <- suppressMessages(lister_exercices())
  expect_setequal(
    codes,
...
test_that("par_ou_commencer() détecte les exercices installés", {
  ...
  skip_if_no_quarto()
  expect_identical(suppressMessages(par_ou_commencer(dest)), "demarrer")
})

test_that("diagnostiquer_rendu() classe l'avertissement de police comme bénin", {
  expect_identical(
    suppressMessages(diagnostiquer_rendu("Error: unknown font family 'Inter'")),
    "benin"
  )
})
...
test_that("diagnostiquer_rendu() repère un Typst absent comme bloquant", {
  expect_identical(
    suppressMessages(diagnostiquer_rendu("Typst executable not found")),
    "bloquant"
  )
})
...
test_that("diagnostiquer_rendu() repère un _brand.yml introuvable", {
  expect_identical(
    suppressMessages(diagnostiquer_rendu("Error: file not found: _brand.yml")),
    "bloquant"
  )
})
...
test_that("diagnostiquer_rendu() repère une erreur de syntaxe YAML", {
  expect_identical(
    suppressMessages(diagnostiquer_rendu("yaml parse error: did not find expected key")),
    "bloquant"
  )
})
