test_that("nettoyer_cache() supprime les artefacts de rendu", {
  ...
  n <- suppressMessages(nettoyer_cache(d))
  expect_gte(n, 3)
...
test_that("ouvrir_exercices() exige un dossier existant", {
  ...
  d <- withr::local_tempdir()
  suppressMessages(installer_exercices(file.path(d, "exos"), quels = "01", force = TRUE))
  expect_identical(
    normalizePath(suppressMessages(ouvrir_exercices(file.path(d, "exos"))), winslash = "/"),
...
test_that("polices_typst() liste des familles", {
  skip_if_no_quarto()
  familles <- suppressMessages(polices_typst())
...
test_that("diagnostic_typst() renvoie une liste", {
  skip_if_no_quarto()
  info <- suppressMessages(diagnostic_typst(withr::local_tempdir()))
...
test_that("inspecter_typ() produit un .typ", {
  ...
  typ <- suppressMessages(inspecter_typ(qmd, ouvrir = FALSE))
