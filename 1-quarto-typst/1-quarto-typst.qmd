...
- **Une charte, deux niveaux** — `_brand.yml` habille **automatiquement** la mise en page (titres, liens, fond, polices). Vos tableaux `gt` et graphes `ggplot` sont du **R** : ils ne suivent que si vous les branchez avec le paquet `brand.yml` (`read_brand_yml()`, `theme_brand_gt()`, `theme_brand_ggplot2()`) — une seule source, un petit geste en plus (détaillé au Bonus B4 du Bloc 2)
