...
5.  Ajoutez la police locale Star Jedi (TTF dans `_fonts/`) sur le titre via `source: file`
...
**Étape 5 (police locale Star Jedi)** : `StarJedi-DGRW.ttf` est déjà dans `starter/_fonts/` (police décorative fan-made, licence freeware via [FontSpace](https://www.fontspace.com/star-jedi-font-f9641)). Au moment de la démo, montrer la syntaxe `source: file` à côté de `source: google` dans le même `_brand.yml` — l'idée à transmettre : « tout type de source coexiste, vous mixez selon vos contraintes ». Expliquer le pourquoi de `typography.title:` (cible le titre principal uniquement, garde Orbitron sur les titres de section pour la lisibilité). Plan B 100 % offline disponible dans `_brand-offline.yml` (toutes les polices en `source: file`) pour les sessions sans réseau.
:::
...
4.  Ajoutez la police locale Star Jedi (`_fonts/StarJedi-DGRW.ttf`, `source: file`) sur le titre
...
**Étape 4 (Star Jedi)** : pour les participants bloqués sur la syntaxe `source: file` + `files:`, montrer rapidement le bloc dans `correction/_brand.yml` au tableau. Ne pas paniquer si certains zappent l'étape — le rendu reste correct sans (titre en Orbitron). C'est l'étape la plus « visuelle » de l'exo : le titre passe en lettres décoratives Star Jedi, effet immédiat.
:::
