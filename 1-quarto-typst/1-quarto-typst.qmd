...
2.  Créez `_brand.yml` **dans le même dossier** (à côté du `.qmd`) avec **une seule couleur** + rôle `primary`, re-rendez → titres et liens colorés
...
- **Une charte, partout** — Côté R, le paquet `brand.yml` lit ce même fichier : `brand <- read_brand_yml()` puis `brand_color_pluck(brand, "ma_couleur")`. Les couleurs de votre PDF, de vos graphes `ggplot` et de vos tableaux `gt` viennent d'une seule source
