...
Les Rencontres R 2026 s'engagent à offrir une conférence **sans harcèlement**, dans un environnement **bienveillant et sûr** pour toutes et tous, quels que soient l'âge, le genre, l'orientation, le handicap, l'apparence, l'origine ou la religion.

::: incremental
- **Être prévenant·e** dans le langage et les actions, respecter les limites de chacun·e
- **S'abstenir** de tout comportement dégradant, discriminatoire ou harcelant
- **En cas de souci** : alerter un membre de l'équipe organisatrice (identifiable au badge), même pour un fait qui semble anodin
:::

::: {.fragment}
Code complet : [code de conduite des Rencontres R 2026](https://rr2026.sciencesconf.org/resource/page/id/13){target="_blank"} · contact [rr2026@sciencesconf.org](mailto:rr2026@sciencesconf.org)
:::

::: notes
Prendre 30 secondes là-dessus, sérieusement mais sans plomber l'ambiance. On anime un atelier : on est responsables du cadre. Rappeler que Maëlle passe dans les rangs et qu'on est tous les deux dispo si quoi que ce soit. Le code de conduite s'applique aussi aux ateliers, pas seulement aux plénières. Texte adapté de useR! 2020 / rOpenSci, sous licence CC BY-SA.
:::
...
![](quarto_typst_pipeline.svg){width="100%" fig-alt="Pipeline en trois étapes : rapport.qmd (source) → Pandoc → rapport.typ (intermédiaire, gardé grâce à keep-typ) → Typst → rapport.pdf (sortie)."}
...
:::: {.fragment .fade-in}
::: {.center style="width:60%; margin:0 auto;"}
```{typst}
...
2.  Créez `_brand.yml` avec **une seule couleur** + rôle `primary`, re-rendez → titres et liens colorés
...
::: notes
C'est la démo live, **format ultra-court** : 5-6 minutes max. Pas-à-pas détaillé (snippets prêts à copier, ressources à ouvrir, pièges anticipés) dans le pas-à-pas démo (doc animateur dédié).

**Pourquoi minimal** : Our turn montre l'effet d'une édition YAML sur le PDF rendu (une couleur suffit), Your turn traduit la charte SW complète. Si Our turn fait tout, Your turn = répétition.
...
**Transition vers Your turn** : « vous avez vu la boucle, à vous avec la charte complète — 4 couleurs au lieu d'1, rôles `foreground` + `background`, Google sur le corps, Star Jedi sur les titres. 12 min. »
:::
...
Gardez votre PDF de l'Exercice 1 sous la main : on réutilise votre `_brand.yml` juste après la pause.
