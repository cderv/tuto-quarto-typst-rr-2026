...
Démo minimale — on plante la **boucle** `_brand.yml` → render → PDF stylé.
...
→ À vous ensuite pour la charte complète (palette + assignments + polices).
:::
...
**Pourquoi minimal** : Our turn plante la boucle (édit YAML → render → effet visible), Your turn transcrit la charte SW complète. Si Our turn fait tout, Your turn = répétition.
...
**Transition vers Your turn** : « vous avez vu la boucle, à vous avec la charte complète — 4 couleurs au lieu d'1, assignments `foreground` + `background`, Google sur le corps, Star Jedi sur les titres. 12 min. »
:::
