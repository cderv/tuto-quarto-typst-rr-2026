...
**Si quelqu'un demande sur le placement du logo** (« il est en haut à gauche, je peux le mettre ailleurs ? ») : le placement par défaut en `format: typst` est `page(background)` haut-gauche, taille 1.5in et padding 0.75in — assez gros pour chevaucher les titres de page 2+ ! Customisation via `format.typst.logo: { path, location, width, padding }` directement dans le YAML doc (Bloc 1) ou dans `_quarto.yml` (Bloc 2). La correction utilise `width: 0.6in` + `padding: 0.4in` pour tenir dans la marge top de 2.5cm. Options `header`/`footer` placement encore en discussion côté Quarto upstream — voir la [discussion #13591](https://github.com/orgs/quarto-dev/discussions/13591).
:::
...
- Palette nommée + **assignments** (`primary`, `foreground`, `background`)
- Inter (Google) + Star Jedi (locale)
- Ce PDF est lui-même rendu par Quarto+Typst+`_brand.yml` — vous allez en faire un cousin
:::
:::
::::::
...
Le terme « assignments » de la charte = mapping `primary` / `foreground` / `background` → noms de palette. Le préciser oralement (la charte n'a pas la place de l'expliquer).
:::
...
4.  Créez un `_brand.yml` en suivant la **charte Star Wars** (`charte-starwars.pdf` du starter) : palette + police Google sur le corps
5.  Ajoutez la police locale Star Jedi (TTF dans `_fonts/`) sur les titres de section via `source: file`
6.  **Bonus** : activez `keep-typ: true` et explorez le fichier `.typ` généré
:::
...
**Avant d'attaquer le YAML** : projeter `_charte/charte-starwars.pdf` à côté de l'éditeur (ou ré-afficher la slide « Voici la charte ») — l'apprenant·e voit cible + transcription en parallèle pendant les étapes 3-5.
...
**Étape 5 (police locale Star Jedi)** : `Starjedi.ttf` est déjà dans `starter/_fonts/` (police décorative fan-made Boba Fonts, archive originale, licence freeware ; les variantes repackagées sur [FontSpace](https://www.fontspace.com/star-jedi-font-f9641) — DGRW / Hollow / Rounded — échouent au subsetting Typst, on retient la TTF originale). Au moment de la démo, montrer la syntaxe `source: file` à côté de `source: google` dans le même `_brand.yml` — l'idée à transmettre : « tout type de source coexiste, vous mixez selon vos contraintes ». Le brand devient Inter (corps, Google) + Star Jedi (titres, locale) — effet visuel immédiat sur chaque titre de section. Plan B 100 % offline disponible dans `_brand-offline.yml` (Inter + Star Jedi, toutes les polices en `source: file`) pour les sessions sans réseau.
:::
...
📄 **Charte fournie** : `charte-starwars.pdf` dans le starter (palette + polices + logo + assignments).
