# Index du paquet

## Vérification & installation

Préparer et contrôler son environnement avant le tutoriel.

- [`verifier_installation()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/verifier_installation.md)
  : Vérifier que votre environnement est prêt pour le tutoriel
- [`par_ou_commencer()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/par_ou_commencer.md)
  : Par où commencer ?

## Pendant l’exercice

Installer, réinitialiser et s’orienter dans les exercices.

- [`installer_exercices()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/installer_exercices.md)
  : Installer les exercices du tutoriel
- [`reinitialiser_exercice()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/reinitialiser_exercice.md)
  : Réinitialiser un exercice à son état de départ
- [`ouvrir_exercices()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/ouvrir_exercices.md)
  : Retrouver et ouvrir le dossier des exercices installés
- [`lister_exercices()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/lister_exercices.md)
  : Lister les exercices du tutoriel

## Polices & charte

Mode hors-ligne, contournement font-paths et validation de `_brand.yml`.

- [`basculer_hors_ligne()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/basculer_hors_ligne.md)
  : Basculer un exercice en mode hors-ligne (polices Inter locales)

- [`appliquer_polices_locales()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/appliquer_polices_locales.md)
  :

  Appliquer le contournement `font-paths` (livre, Quarto \< 1.10.4)

- [`valider_brand()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/valider_brand.md)
  :

  Valider un fichier `_brand.yml`

- [`polices_typst()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/polices_typst.md)
  : Lister les polices vues par Typst

## Dépannage

Décoder les erreurs et inspecter la chaîne Typst.

- [`diagnostiquer_rendu()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/diagnostiquer_rendu.md)
  : Décoder une erreur de rendu Quarto / Typst
- [`diagnostic_typst()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/diagnostic_typst.md)
  : Diagnostic détaillé de la chaîne Quarto / Typst
- [`inspecter_typ()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/inspecter_typ.md)
  : Inspecter le code Typst intermédiaire d'un document
- [`nettoyer_cache()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/nettoyer_cache.md)
  : Nettoyer les artefacts de rendu d'un projet
- [`exporter_diagnostic()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/exporter_diagnostic.md)
  : Exporter un diagnostic d'installation dans un fichier

## Pérennité & après le tutoriel

Réutiliser Quarto + Typst chez vous, explorer les variantes de charte.

- [`creer_projet_typst()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/creer_projet_typst.md)
  : Créer un projet Quarto + Typst réutilisable
- [`basculer_charte()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/basculer_charte.md)
  : Appliquer une variante de charte Star Wars
- [`comparer_chartes()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/comparer_chartes.md)
  : Comparer les couleurs des variantes de charte
- [`ouvrir_correction()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/ouvrir_correction.md)
  : Ouvrir la correction d'un exercice (en ligne)
- [`recuperer_correction()`](https://cderv.github.io/tuto-quarto-typst-rr-2026/package/reference/recuperer_correction.md)
  : Récupérer la correction d'un exercice en local
