...
## 🎯 État de complétude — 16 juin 2026 (à jour 2026-05-04 post-review #2)

### ✅ Livré et stable

- **Site web** : page d'accueil, préparatifs (incl. section « Test de la chaîne Typst » C3), ressources — `quarto render` racine 8/8 OK
- **Bloc 1** (Quarto + Typst sur 1 doc, ~40 min) — slides + page web + Exo 1 starter + correction brandée Star Wars (R chunks `dplyr::starwars`, gt, ggplot, raw `#highlight()`, **helpers `theme_brand_ggplot2()` + `theme_brand_gt()` C1**, **`_logo-sw.svg` en filigrane couverture C2**) + section « À la fin de ce bloc, vous saurez » côté apprenant
- **Bloc 2** (Projet book + brand, ~40 min) — slides (avec `code-line-numbers` 4 fragments sur le YAML étape 2 + **wrap-up 3 slides terminales B1 : « Ce que vous savez faire » / « Et maintenant ? » / « Merci ! Questions ? »**) + page web (modèle `_quarto.yml` complet **avec workaround `font-paths`** + note bug gt + bloc B2 copiable + section « À la fin de ce bloc, vous saurez ») + Exo 2 starter (5 .qmd) + correction book Star Wars complet (préface sans lorem ipsum) + `_brand-starter.yml` + READMEs
...
- **Mini-test Typst end-to-end** (`exercises/00-test-install/test-install.qmd`) référencé dans `preparatifs.qmd` pour validation install pré-tutoriel — détecte version Quarto, fonts fallback, locale R (accents), bug gt cosmétique
- **2 drafts d'issues GitHub prêts** dans `.claude/issues/` — `gh search` confirmé absence de doublon (à filer par CD)
- **4 reviewers workshop persistés** comme subagents Claude Code dans `.claude/agents/` (pédagogue, débutant, technique, FR) — réutilisables sans re-brief
- **Réformulation pédagogique « Concepts clés » → « À la fin de ce bloc, vous saurez »** côté apprenant sur les 2 pages de bloc, en miroir de la slide A du wrap-up
...
- [ ] PDF correction Exo 1 : background crème + jaune SW sur droïdes + gt sans bug espacement + accent label ggplot OK + **logo SW étoile jaune en filigrane haut-gauche première page** (C2) + **panel ggplot brandé via `theme_brand_ggplot2(brand)`** (background crème, axes noir SW, grille adoucie) + **tableau gt brandé via `theme_brand_gt(brand)`** (background crème, texte noir, Inter conservé via `opt_table_font` post-helper) (C1)
- [ ] PDF correction Exo 2 : cover orange-book brandée + Orbitron sur titres + Inter sur corps + Figure 1.1 / Table 1.1 / Annexe A
- [ ] Mini-test C3 : `quarto render exercises/00-test-install/test-install.qmd` produit un PDF avec titre accentué (« Échelle log »), tableau 3 lignes, graphique — sur Linux/macOS/Windows participants
...
- [ ] **Wrap-up Bloc 2 (B1)** — 3 slides finales `2-projets/2-projets.qmd` : (a) « Ce que vous savez faire maintenant » : transitions `incremental` lisibles (1 beat par bullet) ; (b) « Et maintenant ? » : 3 bullets statiques rendent bien ; (c) « Merci ! Questions ? » : signatures CD + Maëlle s'affichent sur 2 lignes distinctes (markdown `**…**\n**…**` rend en 2 paragraphes selon `clean-revealjs`)
...
- 2026-05-04: **Commit 1 livré sur branche `claude/quarto-book-skeleton-qeDNI`** (commits `1b39468` skeleton + `7b0ac7e` workaround font-paths). Plan original mentionnait `claude/add-missing-content-4CdHE` mais cette branche a été mergée → branche actuelle = celle prévue par le harness pour cette session. **Note post-merge** : la branche `claude/quarto-book-skeleton-qeDNI` a depuis été squash-mergée sur `main` ; les SHAs ci-dessus n'existent plus sur `main` (trace historique conservée).
...
- 2026-05-04 (session post-review) : **Polish post-review livré** (commits `aaa4f6b` `1b4e996` `b2d5eb7` `9075d56` `bec9ecc` `607b64f` `4369d21` `5e2563c` `974538c` `65e9c1e` `87969d1` `f7d9956` `0478fd7` `27a0878` — 14 commits + `ebc80c5` gitignore fix). Méthode : 4 reviewers parallèles (pédagogue, élève débutant, Quarto technique, orthographe FR) → consolidation cross-rapports → arbitrage CD → sweep séquentiel. Findings cumulés :
  1. **Décision majeure CD : T1 inversé** — `format: orange-book-typst` n'est nécessaire que pour modifier l'extension. Sinon `format: typst` suffit (orange-book auto-activé sur `type: book`, vérifié `quarto.js:91326-91328`). Tout le matériel pédagogique passé sur `format: typst` (page web, README, slide consigne, correction `_quarto.yml`). **Inverse de la décision documentée plus haut dans ce log** (« Choix final : `format: orange-book-typst` (explicite + correct) » du même jour matin) — la séance review a fait évoluer la position. Plus simple à enseigner (« seul `type: book` change entre étape 1 et 2 »).
  2. **Validation technique critique** : `format.typst.font-paths: [.quarto/typst/fonts]` (sous `format: typst:`, pas `format: orange-book-typst:`) propage bien à orange-book auto-activé. `pdffonts` confirme `Orbitron-Bold` + `Inter-Regular` embedded ; `strace -e openat` confirme typst lit le brand cache. Workaround toujours fonctionnel sous la nouvelle forme.
  3. **Slide deck `3-aller-plus-loin/3-aller-plus-loin.qmd` supprimé** — contenait une slide « Attention au décalage des titres » (L325-336) prétendant que h1 .qmd → `==` (level 2) en Typst. **Faux pour Quarto 1.9** : c'est `=` (level 1) sans décalage, vérifié sur `.typ` générés. Même claim retiré de `3-aller-plus-loin/index.qmd:42` et `4-ressources.qmd:136`. Page web `index.qmd` conserve « Points couverts » comme topic-store.
  4. **Vouvoiement uniforme acté** — décision CD : tableaux Bloc 2 (page web + README) passés du tutoiement (« Crée », « Passe », « Copie », « ajoute ») au vouvoiement (« Créez », « Passez », « Copiez », « ajoutez »). 10 lignes touchées (6 core + 4 bonus).
  5. **Sweep vocabulaire FR (anglicismes en prose → français)** : `brand` → `charte`, `cross-refs` → `références croisées`, `headings` → `titres`, `fontes` → `polices`, `scaffolder` → `générer un squelette`, `Open-Source` → `open source`. ~18 lignes touchées dans 6 fichiers. Garde en code : `_brand.yml` (nom fichier), `brand-mode:` (clé YAML), `quarto use brand` (sous-commande shell), `brand-color.primary` (variables Typst), `headings:` (clé YAML `typography.headings`).
  6. **Cognitive load étape 2 : split 2a/2b** — slide passe en `code-line-numbers="1-2|4-9|10-11|13"` (4 fragments highlightés successifs : `project.type` / `book` block / `appendices` / `format`). Tableau page web + README : étape 2 splittée en 2a (`type: book` + `chapters:` minimal) et 2b (ajout `appendices:`). Le titre « 3 étapes core (12 min) » reste — pédagogiquement c'est toujours 3 concepts (YAML projet, livre relié, charte). 2a/2b est un séquençage interne.
  7. **Lacunes participant comblées** : (a) modèle `_quarto.yml` complet copiable ajouté README + page web (le slide étant fugace) ; (b) note bug gt « 1 7 5 » + workaround `opt_table_font` ajoutée côté participant (était mentionnée seulement en notes presenter) ; (c) bloc B2 `.content-visible`/`pagebreak` sorti de la cellule de tableau vers un bloc de code copiable sous le tableau bonus.
  8. **Cohérence syntaxe `_brand.yml`** — slide démo Bloc 1 passé de la forme objet (`typography.base.family: "Noto Sans"`) à la forme `fonts:` list + `base:` string, alignée sur les 3 `_brand.yml` réels du repo (correction Bloc 1, correction Bloc 2, fallback).
  9. **Pépite Template partials reformulée** — slide L126 disait « Deux fichiers (typst-show.typ et typst-template.typ) ». Or orange-book n'utilise PAS `typst-template.typ` (vérifié `_extension.yml`). Reformulé : `typst-show.typ` seul suffit souvent ; `typst-template.typ` accompagne sur le template par défaut Quarto.
  10. **Orthographe** : `(echelle log)` → `(échelle log)` ×3 fig-cap, `subtitle = "Personnages par planete"` → `planète` ×2 (subtitle gt **non concerné** par le workaround sans-accent qui ne s'applique qu'à `cols_label` à cause du bug `gt → Typst` sur en-têtes de colonne). `« 4 fichiers .qmd »` → `5` (typo README). `scales` ajouté à `preparatifs.qmd:18` (utilisé par `01-anatomie.qmd:60` via `scales::label_number`).
  11. **Issues drafts adaptés** : `format: typst` comme exemple principal (au lieu de `format: orange-book-typst`) puisque c'est la forme désormais utilisée par le matériel. `gh search issues` confirmé absence de doublon côté `quarto-dev/quarto-cli` (`supplement-chapter`, `orange-book lang`, `font-paths book`, `brand fonts book typst`). Drafts prêts à être ouverts par CD.
  12. **Restes pour CD (hors session Claude)** : (a) filing effectif des 2 issues GitHub côté `quarto-dev/quarto-cli` ; (b) vérifications visuelles avant le 16 juin (slide 2-projets `code-line-numbers` highlight bien les bons paquets, callout-warning bug gt avec `collapse=true` rend bien, PDF correction Exo 2 cover orange-book brandée + Orbitron + Inter).
- 2026-05-04 (session post-review #2) : **Vague de fixes priorisée** post-review consolidée vague 2 (4 reviewers parallèles via subagents `.claude/agents/` persistés). 18 commits squash-mergés sur `main` (`9b7a27e` → `9856186`). Méthode : reviewers parallèles → consolidation cross-rapports → arbitrage CD priorité P0 → P1 → P2 → P1 lite. Findings cumulés :
  1. **P0 #1 — Lorem ipsum résiduel** (commit `d55526a`) : préface book Star Wars (`02-projet-book/{starter,correction}/index.qmd:3`) commençait par « Lorem ipsum dolor sit amet, consectetur adipiscing elit. » jamais remplacé. Visible sur la couverture orange-book au rendu PDF. Fix : phrase démarre par « Ce livre explore le dataset `dplyr::starwars`… ».
  2. **P0 #2 — Workaround `font-paths` manquant côté participant** (commit `d55526a`) : modèle `_quarto.yml` proposé au participant (`2-projets/index.qmd:78-99` + `02-projet-book/README.md:31-52`) ne répliquait pas le bloc `format.typst.font-paths: [.quarto/typst/fonts]` présent dans la correction. Sans ce bloc, étape 3 (« charte jaune SW + Orbitron + Inter ») échoue silencieusement chez le participant : `warning: unknown font family: orbitron/inter` + headings en serif fallback. Régression introduite par les commits `5e2563c` puis `34e8d0f` du matin. Modèle participant ↔ correction restaurés.
  3. **B1 — Wrap-up de fin de tutoriel** (commit `d3beea3`) : 3 slides My turn ajoutées **après** la pépite (`2-projets/2-projets.qmd:122-138`), position terminale absolue qui survit même si la pépite saute : (a) « Ce que vous savez faire maintenant » (`::: incremental` 4 bullets miroir des objectifs `index.qmd:17-20`), (b) « Et maintenant ? » (3 bullets : « Cette semaine », « Pour creuser », « Communauté »), (c) « Merci ! Questions ? » (signatures CD + Maëlle, 2 questions stock pré-préparées en notes presenter pour relancer si silence). Note presenter pépite reformulée : « Fusible à couper… sauter directement au wrap-up qui suit (3 slides obligatoires, ne pas sauter) ».
  4. **C1 — Helpers brand.yml dans correction Exo 1** (commit `2fdf8fc`, partiellement reverté `a3be7ef`) : `library(brand.yml)` + `brand <- read_brand_yml()` ajouté au setup chunk de `correction/rapport-starwars.qmd`. Pipeline gt enrichi de `theme_brand_gt(brand)` AVANT `opt_table_font(font = "Inter")` (ordre important : helper écrit `table.background.color` + `table.font.color` mais pas la police, opt_table_font garde Inter en tête pour le workaround chiffres « 1 7 5 »). Pipeline ggplot enrichi de `+ theme_brand_ggplot2(brand)` APRÈS `theme_minimal(base_size = 11)` (pattern additif : theme_minimal pose la base, helper applique la charte SW par-dessus). Promesse README étape 4 réécrite (« Pour propager la charte aux figures ggplot et tableaux gt, utilisez les helpers… »). `4-ressources.qmd:72` enrichi avec liste des 5 helpers brand.yml. **Sub-revert** (`a3be7ef`) : `brand.yml` 0.1.0 EST sur le CRAN depuis 2025-11-06 (faux négatif sandbox via cache pak obsolète). Le split install CRAN+GitHub initialement ajouté à `preparatifs.qmd` a été reverté ; le bloc `install.packages(pkg)` original est correct.
  5. **C2 — Logo SW dans `_brand.yml` Exo 1** (commit `f3760e6`) : `correction/_logo-sw.svg` créé (copie 1:1 du SVG Bloc 2, 342 octets, étoile jaune). Bloc `logo: { images: { sw-star: { path } }, medium: sw-star }` ajouté au `correction/_brand.yml` ET `correction/_brand-offline.yml` pour cohérence Plan B. **Découverte au smoke test** : Quarto consomme DÉJÀ ce bloc en `format: typst` simple via `page(background: align(left+top, box(inset: 0.75in, image(...))))` — logo apparaît en filigrane haut-gauche première page sans chevauchement texte. Renforce le pivot narratif Bloc 1 → Bloc 2 (le participant reconnaît son logo sur la couverture book à l'étape 3). README Exo 1 étapes 4 + Solution mis à jour. Note presenter Bloc 1 enrichie d'une mention optionnelle vers la discussion upstream `quarto-dev/quarto-cli#13591` sur le placement du logo.
  6. **C3 — Mini-test Typst end-to-end** (commit `52d98e4`) : nouveau fichier autonome `exercises/00-test-install/test-install.qmd` (~35 lignes) sans `_brand.yml` (évite la dépendance Google fonts, test fiable derrière firewall corp). 3 chunks R progressifs : setup paquets, gt + `opt_table_font("Arial")` (workaround universel), ggplot avec accent dans le titre (« Échelle log : test des accents »). Section dédiée `## Test de la chaîne Typst` ajoutée à `preparatifs.qmd` entre `## Vérification` et `## Plan B`, avec troubleshooting 5 cas (Quarto < 1.9, fonts fallback, accents cassés `LANG=fr_FR.UTF-8`, réseau, chiffres espacés). `_quarto.yml` inchangé (`!exercises/` et `resources: exercises/**` couvrent déjà). `.gitignore` couvre `test-install.pdf`.
  7. **Lot A — 7 fixes triviaux** (commit `b66900d`) : sans débat, identifiés par reviewers technique (1) + FR (6). (a) `4-ressources.qmd:42` : `quarto typst gather` n'existe pas → `quarto call typst-gather` (vraie syntaxe blog Quarto 2026-03-31). (b) `2-projets/2-projets.qmd:105` : `{{< pagebreak >}}` non échappé → `{{</* pagebreak */>}}`. (c) `02-projet-book/README.md:76` `Cross-refs` → « Références croisées » (résidu sweep). (d) `escape hatch` → « soupape » (×1). (e) `punchline` → « chute » (×2). (f) `outlier(s)` → « valeurs aberrantes » (×6). (g) `Brander.` titre étape 4 Exo 1 README → « Charte. ». (h) `Setup` H2 Exo 2 README → « Mise en place ». (i) `core` (×4 prose participant) → « principales ». (j) `**Fix**` (×2 callouts bug gt) → `**Correction**`. (k) `polices fallback du tableau` (×2) → « polices de remplacement ». (l) `Pré-requis` → `Prérequis` (forme moderne). (m) `sans friction` (×2) → `sans frictions` (cohérent titre canonique workshop).
  8. **Réformulation pédagogique « Concepts clés » → « À la fin de ce bloc, vous saurez »** (commit `9856186`, suite à P1 lite review pédagogue) : `1-quarto-typst/index.qmd:24-29` + `2-projets/index.qmd:24-28`, sections My turn des 2 blocs reformulées en perspective apprenant, bullets en verbes infinitifs (« Produire un PDF », « Régler les options », « Inspecter le pipeline », « Personnaliser… »). Refermement de la boucle pédagogique : promesse au début (« vous saurez ») → validation à la fin (slide A wrap-up B1 « Ce que vous savez faire maintenant »).
  9. **4 reviewers persistés en subagents** (commit `c827139`) : `.claude/agents/workshop-reviewer-{pedagogue,debutant,technique,fr}.md`. Outils : Read, Grep, Bash, Write (Write uniquement pour le rapport — règle stricte « pas de modif sources, pas de commit »). Format de livrable identique : Verdict / P0 / P1 / P2 / Forces / Évolution depuis review précédente. Naming convention `workshop-reviewer-<angle>` cohérent avec `workshop-{pacing,content}.md` skills. Réinvocations futures : passer (a) commit de référence, (b) historique des fixes depuis dernière review, (c) path d'output.
  10. **Reviews vague 2 archivées** (commits `6acfc25` `405c323` `fb77c8a`) : 4 rapports persistés à la racine du repo (`review-2026-05-04-bis-{1-pedagogue,2-eleve-debutant,3-quarto-technique,4-orthographe-fr}.md`). Verdicts cumulés : 0 P0 pédagogue, 1 P0 débutant (font-paths participant), 1 P0 FR (lorem ipsum), 0 P0 technique. Tous adressés cette session.
  11. **Vue macro complétude ajoutée à `PLAN.md`** (commit `062c9b2`) : section « 🎯 État de complétude » en haut du fichier (avant decision log) avec 5 sous-blocs (✅ Livré / 🟡 Bloc 3 réduit / 🔴 Restes CD / 🟢 Optionnel / 📦 Post-mortem). Permet à une nouvelle session de voir l'état global sans reconstituer depuis l'historique.
  12. **Audit tracking docs `.claude/`** (commit `9b7a27e`) : passe ciblée pour aligner `PLAN.md` (entrée « session post-review » du matin), `project-context.md` (slide deck 3-aller-plus-loin retiré), `topic-store.md` (mentions `typst-template.typ` nuancées), `exo2-book.md` (en-tête archivé). C'est cette même approche qui sera répétée dans la session post-review #2 (i.e. *cette* entrée).
- 2026-05-25 : **Charte graphique Star Wars livrée en PDF dans les starters** (`charte-starwars.pdf` dans `exercises/01-document-typst/starter/` + `exercises/02-projet-book/starter/`). Source : nouveau dossier `_charte/` à la racine (`.qmd` + `_brand.yml` + `_logo-sw.svg` + `_fonts/Starjedi.ttf` + `README.md` pipeline manuel). Motivation : sur 2 h l'objectif est de transcrire une identité en YAML, pas de la designer. Avant, l'étape 3 Exo 1 disait « `color: { primary: "#…" }` (+ `secondary` au choix) » → l'apprenant inventait, divergence des rendus, discussion plénière diluée. La charte donne palette nommée + hex + assignments (`primary: imperial-red`, `foreground: sw-black`, `background: sw-cream`) + polices (Inter Google, Star Jedi locale) + logo (`_logo-sw.svg` + alt). **Méta-bonus** : la charte est elle-même un PDF Typst rendu via `_brand.yml` — le logo en haut-gauche est placé automatiquement par Quarto via `logo.medium`, ce qui montre dès la lecture de la charte l'effet visuel de la déclaration logo. Mises à jour matériel : `1-quarto-typst/index.qmd` (étape 3 réécrite + ligne « 📄 Charte fournie » dans le callout « À vous »), `2-projets/index.qmd` (étape 3 mentionne la charte fallback), starter READMEs des 2 exos (ligne ajoutée au tableau « Contenu du dossier »), slide « Faisons ensemble » + slide « À vous » Bloc 1, `correction/README.md` Exo 1 (mention que le `_brand.yml` est la transcription fidèle de la charte). Précédents consultés : `cderv/rr2025-quarto-brand-yml` (Pulse Mobile fournie) et `gadenbuie/slides/brand-yml-shiny` (AI@T-Mobile fournie) — confirment le pattern « marque clé-en-main, audience transcrit ».
...
- [x] **Lien Exo 2 actuellement 404** dans `2-projets/index.qmd:44` — résolu 2026-05-04 (matériel livré Commit 4 ; mention « (matériel finalisé avant le 16 juin) » retirée du texte Commit 5). Branche `claude/quarto-book-skeleton-qeDNI` mergée sur `main` post-session ; le lien `tree/main/exercises/02-projet-book/starter/` résout désormais.
...
- [ ] `exercises/02-projet-book/_brand-starter.yml` pour ceux qui n'ont pas fini Exo 1
- [ ] README.md exo 2
...
- [ ] Push sur la branche de travail courante (le harness re-attribue une branche par session ; ex. historique : `claude/review-main-integration-pNEI1` 2026-05-02, `claude/quarto-book-skeleton-qeDNI` 2026-05-04 matin, `claude/audit-workshop-docs-GM1AB` 2026-05-04 après-midi). Le nom exact figure dans l'instruction harness en début de session.
