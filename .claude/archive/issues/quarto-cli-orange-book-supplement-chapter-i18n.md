...
Caught while building workshop material for *Rencontres R 2026 — PDF sans frictions : Typst dans vos projets Quarto* (correction in [cderv/tuto-quarto-typst-rr-2026](https://github.com/cderv/tuto-quarto-typst-rr-2026), branch `claude/quarto-book-skeleton-qeDNI`, file `exercises/02-projet-book/correction/`). Le tutoriel pousse `format: typst` (auto-activation orange-book) côté book — bug visible pour tout participant francophone avec `lang: fr` correctement set.
