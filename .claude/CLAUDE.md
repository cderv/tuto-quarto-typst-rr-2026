...
- Plancher : Quarto 1.9+ ; **pre-release `v1.10.7+` recommandée** (la dernière). Le fix brand fonts book ([quarto-dev/quarto-cli#14517](https://github.com/quarto-dev/quarto-cli/pull/14517)) est livré depuis `v1.10.4` : sur Quarto < `v1.10.4`, l'exercice 2 doit appliquer le workaround `font-paths` documenté dans le callout des supports. Dans le paquet R, distinguer `.quarto_reco` (1.10.7, version recommandée) de `.quarto_fix` (1.10.4, borne du correctif polices).
...
- **`_typst-packages/`** (racine, committé) = copie du cache Typst (`fletcher`, `cetz`, `oxifmt`, versions épinglées). Évite tout téléchargement `@preview` au build → **offline et reproductible**. Le préfixe `_` garde le dossier hors des entrées de rendu Quarto. Mettre à jour : éditer `typst-gather.toml` (versions `[preview]`), puis `quarto call typst-gather` — l'outil télécharge fletcher et ses dépendances transitives (cetz, oxifmt) directement dans `_typst-packages/`.
...
- **`pkg/inst/` = COPIE générée** depuis `exercises/` (starters + **sources des `correction/`** + **assets racine des exos** : `_brand-starter.yml`, `_logo-sw.svg`, `_brand-offline.yml`, `_fonts/` — posés par `installer_exercices()` pour le fallback sans Bloc 1 / le plan B hors-ligne — + `templates/brands/` + `offline/_fonts/`) par `pkg/data-raw/sync-exercices.R` ; renommage `exercises/`→`exercices/` **assumé**. Régénérer : `just pkg-sync` ; vérifier : `just pkg-sync-check` (même garde-fou en CI, `pkg-inst-sync.yml`). **Committé** (r-universe build par `git clone`). Corrections embarquées **jamais** posées par `installer_exercices()` (starters only) : copie locale **opt-in** via `recuperer_correction()` (confirmation) ; consultation en ligne via `ouvrir_correction()`.
...
- **Procédures détaillées (progressive disclosure)** : publication r-universe → `pkg/dev/PUBLICATION-r-universe.md` (le `packages.json` à poser dans le repo de registre `cderv.r-universe.dev` y est **inliné**, pas un fichier séparé) ; tests manuels RStudio → `pkg/dev/TESTS-MANUELS.md`. Dév local : `pak::pak("local::./pkg")`.
- **Versions & changelog — `CHANGELOG.md` ↔ `pkg/NEWS.md` (anti-dérive, IMPORTANT)** : deux fichiers, même numérotation (celle du paquet). `CHANGELOG.md` (racine) = **niveau projet** (site + paquet, format Keep a Changelog) ; `pkg/NEWS.md` = **détail paquet** (convention R). **Règle** : tout changement notable ou bump de version se documente **dans les DEUX**, avec le même numéro de version, et le bump de `pkg/DESCRIPTION` accompagne les deux. Le CHANGELOG renvoie vers `NEWS.md` pour le détail paquet — ne pas laisser l'un avancer sans l'autre.
...
- **Pages exercice — `index.qmd` ↔ `boussole.qmd` (un couple par bloc)** : `index.qmd` = vue d'ensemble + **détail de référence** (tableaux d'étapes « Vous devriez voir », modèle `_quarto.yml`, bonus, indices doc, correction) = **source de vérité**. `boussole.qmd` = carte condensée second écran (étapes télégraphiques + countdown) qui **renvoie** vers index. **Règle anti-dérive (IMPORTANT)** : dès que l'un des deux est modifié, **vérifier et resynchroniser l'autre du même bloc** — le détail se met d'abord dans `index.qmd`, puis se répercute dans `boussole.qmd` (étapes, numérotation type 2a/2b, chrono, objectif). Garder la boussole en pointeurs courts, jamais en ré-explications. La dérive arrive vite (ex. `headings.color: primary` ajouté dans index mais oublié dans la boussole).
