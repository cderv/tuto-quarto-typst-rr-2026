...
- **Bloc 2** (Projet book + brand, ~40 min) — slides (avec `code-line-numbers` 4 fragments sur le YAML étape 2 + **wrap-up 3 slides terminales B1 : « Ce que vous savez faire » / « Et maintenant ? » / « Merci ! Questions ? »**) + page web (modèle `_quarto.yml` complet **avec workaround `font-paths`** + note bug gt + bloc B2 copiable + section « À la fin de ce bloc, vous saurez ») + Exo 2 starter (5 .qmd) + correction book Star Wars complet (préface sans lorem ipsum) + `_brand-fallback.yml` + READMEs
