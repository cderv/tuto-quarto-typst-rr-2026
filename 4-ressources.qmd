...
- `basculer_hors_ligne()`, `valider_brand()`, `diagnostiquer_rendu()` — confort et dépannage
...
- Accéder aux couleurs depuis R : `brand_color_pluck(brand, "primary")` ou `brand_color_pluck(brand, "ma_couleur")` — pour styler finement un tableau `gt` ou un graphe sans dupliquer les valeurs hex (visible dans `exercises/01-document-typst/correction/rapport-starwars.qmd`). Attention, les clés palette `tiret-séparé` sont normalisées en `tiret_souligné` au read.
